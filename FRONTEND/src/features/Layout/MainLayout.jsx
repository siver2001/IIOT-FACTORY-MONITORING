...
import { useAlertManagement } from '../../hooks/useAlertManagement'; // <-- ĐÃ THÊM
...
 * @param {number} activeAlertCount Số lượng cảnh báo active (Chỉ Active/Chưa xác nhận)
...
        const hasActiveAlert = activeAlertCount > 0; // Sử dụng count đã được lọc
        
        if (isAlertRoot) {
            if (hasActiveAlert) {
...
    const { kpiSummary } = useAlertManagement();
    
    // Logic 1: Menu Chớp đỏ (chỉ Active/Chưa xác nhận)
    const activeAlertCount = kpiSummary.activeCount;
    
    // Logic 2: Badge Header (Active + Acknowledged)
    const unResolvedAlertsCount = kpiSummary.activeCount + kpiSummary.acknowledgedCount;
...
                    <Space size="large">
                        
                        {/* SỬ DỤNG unResolvedAlertsCount CHO BADGE HEADER */}
                        <Badge count={unResolvedAlertsCount} offset={[5, 0]} dot={unResolvedAlertsCount > 0}>
                            <AlertOutlined style={{ fontSize: '24px', color: unResolvedAlertsCount > 0 ? '#ff4d4f' : '#000' }} />
                        </Badge>
